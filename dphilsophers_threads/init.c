...
/*   Updated: 2023/06/05 10:41:25 by rghouzra         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_philo_mutexes(t_philo **philo, t_philos_table *table, int i)
{
	...
	(*philo)->right_fork = (*table).forks[(i + 1) * (((*table).forks[i
				+ 1] != NULL) + ((*table).params.nb_philos == 1))];
...
void	init_mutexes(t_philos_table *table)
{
	...
	(*table).meals = malloc(sizeof(pthread_mutex_t *)
			* (*table).params.nb_philos);
	(*table).eat_counts = malloc(sizeof(pthread_mutex_t *)
			* (*table).params.nb_philos);
